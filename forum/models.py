...
class PostManager(models.Manager):
    def get_visible_posts(self):
        return self.get_queryset().filter(is_removed=False, topic__is_removed=False)
...
class Permission(models.Model):
    ...
    def can_change_topic(self, user, topic):
        ...
        if self.can_change_group_topic:
            return self._has_priority(topic.first_post.profile, user.profile)
    ...
    def can_change_post(self, user, post):
        ...
        if self.can_change_group_post:
            return self._has_priority(post.profile, user.profile)
